...
export async function getInstances(endpointUrl: string, pageQuery: string) {
...
export async function deleteInstanceRequest(endpointUrl: string, instancePk: string) {
  await api.delete(`${endpointUrl}${instancePk}`)
}

export async function updateInstanceDetailsRequest(
  endpointUrl: string,
  instancePk: string,
  body: object,
