...
const { headings, permissions } = handleComponent.currentPageDependencies

watchEffect(async () => {
  handleComponent.setStateData([])
  handleComponent.isLoading = true
  ...
  const endpointQuery = `?${sortQuery}${searchFilterQuery}${pageQuery}`
  await handleComponent.getRecords(endpointQuery)
})
...
<template>
  <div>
    <CreateInstanceComponent
      v-if="handleComponent.showCreateInstancePopup && toAdd"
      :modal-name="route.meta.name"
    />

    <ViewInstanceComponent
      v-if="handleComponent.showViewInstanceDetailsPopup"
      :modal-title="`${route.meta.name} Details`"
    />
    <UpdateInstanceComponent
      v-if="handleComponent.showUpdateInstancePopup && toChange"
      :modal-name="route.meta.name"
    />
    <DeleteInstanceComponent
      v-if="handleComponent.showDeleteInstancePopup && toDelete"
      :modal-title="`Delete ${route.meta.name}`"
    />

    <FilterListComponent
      v-if="handleComponent.showFilterInstancesPopup"
      :modal-name="route.meta.name"
    />

    <FileExplorerPopup
      v-if="handleComponent.showFolderExplorerPopup"
    ...
    <div class="datatable-toolbar">
      ...
      <VButtons>
        <VButton
          ...
          outlined
          @click="handleComponent.showFilterInstancesPopup = true"
          >Filters</VButton
        >
        <VButton
          ...
          icon="feather:plus"
          elevated
          @click="handleComponent.getCreateInstancePopup()"
        >
          Create Record
        </VButton>
      </VButtons>
    ...
    <div class="datatable-wrapper">
      <VLoader :active="handleComponent.isLoading" size="large">
        <div v-if="handleComponent.isLoading" class="placeholder-loader"></div>
        <div v-else class="table-container">
          <table class="table datatable-table is-fullwidth">
            ...
            <tbody v-if="!handleComponent.isLoading">
              <tr v-for="record in handleComponent.currentStateData" :key="record.id">
                <td v-for="col in headings" :key="col.key">
                  <TableActionsDropdown
                    v-if="col.data == 'Actions'"
                    :to-change="toChange"
                    :to-delete="toDelete"
                    :instance="record"
                  />
                  ...
                  <div v-else-if="col.data == 'hours'">
                    <VButton
                      v-if="
                        handleTimeEntries.isLoading &&
                        handleTimeEntries.loadingTimeEntry == record
                      "
                      class="timer-button"
                      placeload="67px"
                      color="primary"
                    ...
                    <VButton
                      ...
                      rounded
                      :icon="
                        timeEntriesHelpers.getCorrectCounterIcon(record['is_counting'])
                      "
                      elevated
                      @click="handleTimeEntries.toggleTimeEntryCounter(record)"
                    >
                      {{
                        formatting.getFormattedTime(shared.flattenObj(record)[col.data])
                      }}
                    </VButton>
                  </div>
      ...
      <VPlaceholderPage
        v-if="handleComponent.dataLength === 0 && !handleComponent.isLoading"
    ...
    <!--Table Pagination-->
    <VFlexPagination
      v-if="handleComponent.dataLength > 5"
      v-model:current-page="queryParam.page"
      :item-per-page="handleComponent.itemPerPage"
      :total-items="handleComponent.dataLength"
